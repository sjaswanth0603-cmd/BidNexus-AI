...
def extract_pdf_chunks_from_bytes(content: bytes, document_id: str, file_name: str) -> List[Dict[str, Any]]:
    """
    Extracts text from in-memory PDF bytes page-by-page using pypdf or PyMuPDF.
    """
    chunks = []
    try:
        pages_text = []
        if pypdf:
            ...
            for page_num in range(len(doc)):
                pages_text.append(doc[page_num].get_text("text").strip())
            doc.close()
        else:
            pages_text.append(f"Document content from {file_name}")

        for page_num, text in enumerate(pages_text, start=1):
            header = f"[Document File: {file_name} | Page: {page_num}]\n"
